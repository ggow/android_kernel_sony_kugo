...
extern void mmc_log_cmd_history(struct mmc_host *host, struct mmc_request *mrq, int state, int err);
...
#define CMDQ_ENABLE_START_ID	0xFF05
#define CMDQ_ENABLE_END_ID	0xFF06
#define CMDQ_DISABLE_START_ID	0xFF07
#define CMDQ_DISABLE_END_ID	0xFF08

	if (enable) {
		mmc_log_cmd_history(card->host, NULL, CMDQ_ENABLE_START_ID, 0);
	} else {
		mmc_log_cmd_history(card->host, NULL, CMDQ_DISABLE_START_ID, 0);
	}
	...
	if (enable) {
		mmc_log_cmd_history(card->host, NULL, CMDQ_ENABLE_END_ID, ret);
	} else {
		mmc_log_cmd_history(card->host, NULL, CMDQ_DISABLE_END_ID, ret);
	}
...
static enum blk_eh_timer_return mmc_blk_cmdq_req_timed_out(struct request *req)
{
	...
	mmc_log_cmd_history(host, mrq, DBG_REQ_STATE_SW_TIMEOUT, -ETIMEDOUT);
