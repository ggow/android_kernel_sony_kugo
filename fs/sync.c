...
static
#endif
void sync_inodes_one_sb(struct super_block *sb, void *arg)
{
	if (!(sb->s_flags & MS_RDONLY))
		sync_inodes_sb(sb);
}
#ifdef CONFIG_DYNAMIC_FSYNC
EXPORT_SYMBOL_GPL(sync_inodes_one_sb);
#endif

#ifndef CONFIG_DYNAMIC_FSYNC
static
#endif
void sync_fs_one_sb(struct super_block *sb, void *arg)
{
	if (!(sb->s_flags & MS_RDONLY) && sb->s_op->sync_fs)
		sb->s_op->sync_fs(sb, *(int *)arg);
}
#ifdef CONFIG_DYNAMIC_FSYNC
EXPORT_SYMBOL_GPL(sync_fs_one_sb);
#endif

#ifndef CONFIG_DYNAMIC_FSYNC
static
#endif
void fdatawrite_one_bdev(struct block_device *bdev, void *arg)
{
	filemap_fdatawrite(bdev->bd_inode->i_mapping);
}
#ifdef CONFIG_DYNAMIC_FSYNC
EXPORT_SYMBOL_GPL(fdatawrite_one_bdev);
#endif
...
#ifdef CONFIG_DYNAMIC_FSYNC
EXPORT_SYMBOL_GPL(fdatawait_one_bdev);
#endif
